...
async function setup() {
    const patchExportURL = "export/windchimes.export.json";
    ...
    let dependencies = [];
    try {
        const dependenciesResponse = await fetch("export/dependenciesDW.json");
...
class Sensor {
    ...
    figureOutChannel() {
        if (this.name.includes("1")) {
            ...
            this.className = 'device-block-unknown'
        }
